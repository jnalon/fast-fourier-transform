...
{
    double complex W, Wk, Wkn;                 // Twiddle factors;

    W = cexp(-2i*M_PI/N);                      // Initialize twiddle factors;
    Wk = 1;
    for(int k=0; k<N; k++) {
...
{
    double complex *xe, *xo, *Xe, *Xo;         // Vectors with intermediate results;
    double complex W, Wk, WkXo;                // Twiddle factors;
    int N2;
    ...
    if(N==1)                                   // A length-1 vector is its own FT;
        ...
        xe = (double complex *) malloc(N2 * sizeof(double complex));     // Allocate memory;
        xo = (double complex *) malloc(N2 * sizeof(double complex));
        Xe = (double complex *) malloc(N2 * sizeof(double complex));
        Xo = (double complex *) malloc(N2 * sizeof(double complex));
        ...
        double_recursive_fft(xe, Xe, N2);      // Transform of even samples;
        double_recursive_fft(xo, Xo, N2);      // Transform of odd samples;

        W = cexp(-2i*M_PI/N);                  // Twiddle factors;
        Wk = 1;
        for(int k=0; k<N2; k++) {
            WkXo = Wk * Xo[k];
            X[k] = Xe[k] + WkXo;
...
{
    double complex W, Wkn;                      // Twiddle factors;
    int r, l, p, q, step;
    ...
    for(int k=0; k<r; k++) {
        for(int l=0; l<N; l+=2*step) {
            W = cexp(-1i*M_PI/step);           // Twiddle factors;
            Wkn = 1;
...
{
    double complex *xj, *Xj;                    // Subsequences of the transform;
    double complex W, Wj, Wkj;
    int N1, N2;

    N1 = factor(N);                            // Smallest prime factor of the length;
    if (N1==N)                                 // If the length is prime itself,
        double_direct_ft(x, X, N);             //   the transform is given by the direct form;
    else {
        N2 = N / N1;                           // Decompose in two factors, N1 being prime;

        xj = malloc(N2 * sizeof(double complex));       // Allocate memory for subsequences
        Xj = malloc(N2 * sizeof(double complex));       //    and their transforms;

        W = cexp(-2i*M_PI/N);                  // Twiddle factor;
        Wj = 1;
        for(int j=0; j<N1; j++) {              // Compute every subsequence of size N2;
            ...
            double_recursive_nfft(xj, Xj, N2); // Compute the DFT of the subsequence;
            Wkj = 1;
            for(int k=0; k<N; k++) {
